...
struct Reconstruct<Fluid::euler, Reconstruction::weno5> {
  ...
  Solve(parthenon::team_mbr_t const &member, const int k, const int j, const int il,
    ...
    parthenon::par_for_inner(member, il, iu, [&](const int i) {
      ...
      Real max_eig_0 = std::max({std::abs(vex0-a0), std::abs(vex1-a1), std::abs(vex2-a2), std::abs(vex3-a3), std::abs(vex4-a4), std::abs(vex5-a5)});
      Real max_eig_1 = std::max({std::abs(vex0),    std::abs(vex1),    std::abs(vex2),    std::abs(vex3),    std::abs(vex4),    std::abs(vex5)});
      Real max_eig_2 = std::max({std::abs(vex0+a0), std::abs(vex1+a1), std::abs(vex2+a2), std::abs(vex3+a3), std::abs(vex4+a4), std::abs(vex5+a5)});
      ...
      for (int iii = 0; iii < NHYDRO; ++iii) {
        ...
        // g_p5[iii] = 0.5 * (gj5[iii] + 1.1 * alpha[iii] * vj5[iii]);   // not needed for WENO
