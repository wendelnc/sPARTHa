...
#ifndef MHD_WENO3_HPP_
#define MHD_WENO3_HPP_
...
template <>
struct Reconstruct<Fluid::mhd, Reconstruction::weno3> {
  ...
  Solve(parthenon::team_mbr_t const &member, const int k, const int j, const int il,
    ...
    parthenon::par_for_inner(member, il, iu, [&](const int i) {
      ...
      Real q0[(NMHD)], q1[(NMHD)], q2[(NMHD)], q3[(NMHD)];
      Real f0[(NMHD)], f1[(NMHD)], f2[(NMHD)], f3[(NMHD)];
      Real rr[(NMHD)][(NMHD)], ru[(NMHD)][(NMHD)];
      Real lu[(NMHD)][(NMHD)], lq[(NMHD)][(NMHD)];
      Real vj0[NMHD], vj1[NMHD], vj2[NMHD], vj3[NMHD];
      Real gj0[NMHD], gj1[NMHD], gj2[NMHD], gj3[NMHD];
      Real g_p0[NMHD], g_p1[NMHD], g_p2[NMHD], g_p3[NMHD];
      Real g_m0[NMHD], g_m1[NMHD], g_m2[NMHD], g_m3[NMHD];
      Real weno_sum[NMHD];
      Real f_half[NMHD];
      ...
      if (ivx == IV1){
        ...
        q0[IB1] = cons(iBx, k, j, i - 2);
        q0[IB2] = cons(iBy, k, j, i - 2);
        q0[IB3] = cons(iBz, k, j, i - 2);
        ...
        q1[IB1] = cons(iBx, k, j, i - 1);
        q1[IB2] = cons(iBy, k, j, i - 1);
        q1[IB3] = cons(iBz, k, j, i - 1);
        ...
        q2[IB1] = cons(iBx, k, j, i);
        q2[IB2] = cons(iBy, k, j, i);
        q2[IB3] = cons(iBz, k, j, i);
      ...
      if (ivx == IV2){
        ...
        q0[IB1] = cons(iBx, k, j - 2, i);
        q0[IB2] = cons(iBy, k, j - 2, i);
        q0[IB3] = cons(iBz, k, j - 2, i); 
        ...
        q1[IB1] = cons(iBx, k, j - 1, i);
        q1[IB2] = cons(iBy, k, j - 1, i);
        q1[IB3] = cons(iBz, k, j - 1, i);
      ...
      if (ivx == IV3){
        ...
        q0[IB1] = cons(iBx, k - 2, j, i);
        q0[IB2] = cons(iBy, k - 2, j, i);
        q0[IB3] = cons(iBz, k - 2, j, i);
        ...
        q1[IB1] = cons(iBx, k - 1, j, i);
        q1[IB2] = cons(iBy, k - 1, j, i);
        q1[IB3] = cons(iBz, k - 1, j, i);
      ...
      Real half_Bx  = 0.5 * (q1[IB1] + q2[IB1]);
      Real half_By  = 0.5 * (q1[IB2] + q2[IB2]);
      Real half_Bz  = 0.5 * (q1[IB3] + q2[IB3]);
      ...
      Real max_eig_1 = std::max({em, std::abs(vex0),      std::abs(vex1),      std::abs(vex2),      std::abs(vex3)});
      Real max_eig_2 = std::max({em, std::abs(vex0),      std::abs(vex1),      std::abs(vex2),      std::abs(vex3)});
      Real max_eig_3 = std::max({em, std::abs(vex0+cax0), std::abs(vex1+cax1), std::abs(vex2+cax2), std::abs(vex3+cax3)});
      Real max_eig_4 = std::max({em, std::abs(vex0-cax0), std::abs(vex1-cax1), std::abs(vex2-cax2), std::abs(vex3-cax3)});
      Real max_eig_5 = std::max({em, std::abs(vex0+cfx0), std::abs(vex1+cfx1), std::abs(vex2+cfx2), std::abs(vex3+cfx3)});
      Real max_eig_6 = std::max({em, std::abs(vex0-cfx0), std::abs(vex1-cfx1), std::abs(vex2-cfx2), std::abs(vex3-cfx3)});
      Real max_eig_7 = std::max({em, std::abs(vex0+csx0), std::abs(vex1+csx1), std::abs(vex2+csx2), std::abs(vex3+csx3)});
      Real max_eig_8 = std::max({em, std::abs(vex0-csx0), std::abs(vex1-csx1), std::abs(vex2-csx2), std::abs(vex3-csx3)});
