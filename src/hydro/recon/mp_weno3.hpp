...
template <>
struct MPReconstruct<Fluid::mhd, Reconstruction::weno3> {

  // HJ-FLUX Function
  KOKKOS_INLINE_FUNCTION
  static void HJ_FLUX(Real &u_x_plus, Real &u_x_minus, 
    const Real &q_im2, const Real &q_im1, const Real &q_i,
    const Real &q_ip1, const Real &q_ip2, const Real &dx) {

      // Compute the First Derivatives
      Real der0 = (q_im1 - q_im2) / dx; // ∆^+ q_{i-2} = q_{i-1} - q_{i-2} / ∆x
      Real der1 = (q_i   - q_im1) / dx; // ∆^+ q_{i-1} = q_{i}   - q_{i-1} / ∆x
      Real der2 = (q_ip1 - q_i)   / dx; // ∆^+ q_{i}   = q_{i+1} - q_{i}   / ∆x
      Real der3 = (q_ip2 - q_ip1) / dx; // ∆^+ q_{i+1} = q_{i+2} - q_{i+1} / ∆x

      // Compute the WENO Reconstruction
      u_x_plus  = WENO3(der3,der2,der1);
      u_x_minus = WENO3(der0,der1,der2);  
  }
  ...
  static TaskStatus CalculateHJFluxes2D(std::shared_ptr<MeshData<Real>> &md) {
    ...
    KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
      ...
      HJ_FLUX(Az_xp,Az_xm,cons(IA3,k,j,i-2),cons(IA3,k,j,i-1),cons(IA3,k,j,i),cons(IA3,k,j,i+1),cons(IA3,k,j,i+2),dx);

      Real Az_yp, Az_ym; // A^{z+}_{yijk} and A^{z-}_{yijk}
      HJ_FLUX(Az_yp,Az_ym,cons(IA3,k,j-2,i),cons(IA3,k,j-1,i),cons(IA3,k,j,i),cons(IA3,k,j+1,i),cons(IA3,k,j+2,i),dy);

      Real max_vex = std::max({std::abs(prim(IV1,k,j,i-2)),std::abs(prim(IV1,k,j,i-1)),std::abs(prim(IV1,k,j,i)),std::abs(prim(IV1,k,j,i+1)),std::abs(prim(IV1,k,j,i+2))});
      Real max_vey = std::max({std::abs(prim(IV2,k,j-2,i)),std::abs(prim(IV2,k,j-1,i)),std::abs(prim(IV2,k,j,i)),std::abs(prim(IV2,k,j+1,i)),std::abs(prim(IV2,k,j+2,i))});
  ...
  static TaskStatus HJAfterstep2D(std::shared_ptr<MeshData<Real>> &md) {
    ...
    KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
      ...
      Real B_x = (1 / (2 * dy)) * (cons(IA3,k,j+1,i) - cons(IA3,k,j-1,i));
      Real B_y = (1 / (2 * dx)) * (cons(IA3,k,j,i-1) - cons(IA3,k,j,i+1));
  ...
  static TaskStatus CalculateHJFluxes3D(std::shared_ptr<MeshData<Real>> &md, const Real dt) {
    ...
    KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
      ...
      HJ_FLUX(Ax_xp,Ax_xm,cons(IA1,k,j,i-2),cons(IA1,k,j,i-1),cons(IA1,k,j,i),cons(IA1,k,j,i+1),cons(IA1,k,j,i+2),dx);

      Real Ay_xp, Ay_xm; // A^{y+}_{xijk} and A^{y-}_{xijk}
      HJ_FLUX(Ay_xp,Ay_xm,cons(IA2,k,j,i-2),cons(IA2,k,j,i-1),cons(IA2,k,j,i),cons(IA2,k,j,i+1),cons(IA2,k,j,i+2),dx);
      ...
      HJ_FLUX(Ax_yp, Ax_ym,cons(IA1,k,j-2,i),cons(IA1,k,j-1,i),cons(IA1,k,j,i),cons(IA1,k,j+1,i),cons(IA1,k,j+2,i),dy);

      Real Ay_yp, Ay_ym; // A^{y+}_{yijk} and A^{y-}_{yijk}
      HJ_FLUX(Ay_yp, Ay_ym,cons(IA2,k,j-2,i),cons(IA2,k,j-1,i),cons(IA2,k,j,i),cons(IA2,k,j+1,i),cons(IA2,k,j+2,i),dy);
      ...
      Real max_vez = std::max({std::abs(prim(IV3,k-2,j,i)),std::abs(prim(IV3,k-1,j,i)),std::abs(prim(IV3,k,j,i)),std::abs(prim(IV3,k+1,j,i)),std::abs(prim(IV3,k+2,j,i))});

      Real Ax_zp, Ax_zm; // A^{x+}_{zijk} and A^{x-}_{zijk}
      HJ_FLUX(Ax_zp, Ax_zm,cons(IA1,k-2,j,i),cons(IA1,k-1,j,i),cons(IA1,k,j,i),cons(IA1,k+1,j,i),cons(IA1,k+2,j,i),dz);

      Real Ay_zp, Ay_zm; // A^{y+}_{zijk} and A^{y-}_{zijk}
      HJ_FLUX(Ay_zp, Ay_zm,cons(IA2,k-2,j,i),cons(IA2,k-1,j,i),cons(IA2,k,j,i),cons(IA2,k+1,j,i),cons(IA2,k+2,j,i),dz);

      Real Az_zp, Az_zm; // A^{z+}_{zijk} and A^{z-}_{zijk}
      HJ_FLUX(Az_zp, Az_zm,cons(IA3,k-2,j,i),cons(IA3,k-1,j,i),cons(IA3,k,j,i),cons(IA3,k+1,j,i),cons(IA3,k+2,j,i),dz);
  ...
  static TaskStatus HJAfterstep3D(std::shared_ptr<MeshData<Real>> &md) {
    ...
    KOKKOS_LAMBDA(const int b, const int k, const int j, const int i) {
      ...
      Real B_y = (1 / (2 * dz)) * (cons(IA1,k+1,j,i) - cons(IA1,k-1,j,i)) -
                (1 / (2 * dx)) * (cons(IA3,k,j,i+1) - cons(IA3,k,j,i-1));

      Real B_z = (1 / (2 * dx)) * (cons(IA2,k,j,i+1) - cons(IA2,k,j,i-1)) -
