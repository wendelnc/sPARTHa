...
template <>
struct MPReconstruct<Fluid::mhd, Reconstruction::weno5> { 
  ...
  static void HJ_FLUX(Real &u_x_plus, Real &u_x_minus, 
    ...
    const Real &q_ip1, const Real &q_ip2, const Real &q_ip3, const Real &dx) {
      ...
      Real der0 = (q_im2 - q_im3) / dx; // ∆^+ q_{i-3} = q_{i-2} - q_{i-3} / ∆x
      Real der1 = (q_im1 - q_im2) / dx; // ∆^+ q_{i-2} = q_{i-1} - q_{i-2} / ∆x
      Real der2 = (q_i   - q_im1) / dx; // ∆^+ q_{i-1} = q_{i}   - q_{i-1} / ∆x
      Real der3 = (q_ip1 - q_i)   / dx; // ∆^+ q_{i}   = q_{i+1} - q_{i}   / ∆x
      Real der4 = (q_ip2 - q_ip1) / dx; // ∆^+ q_{i+1} = q_{i+2} - q_{i+1} / ∆x
      Real der5 = (q_ip3 - q_ip2) / dx; // ∆^+ q_{i+2} = q_{i+3} - q_{i+2} / ∆x

      // Compute the WENO Reconstruction
      u_x_plus  = WENO5(der5,der4,der3,der2,der1);
      u_x_minus = WENO5(der0,der1,der2,der3,der4);
